...
@Service
public class Customer Products and ServicesRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Customer Products and ServicesRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customer products and services.hosturl}")
	private String url;
	
	@Value("${spring.api.customer products and services.context-path}")
	private String contextPath;

	@Value("${spring.api.customer products and services.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Customer Products and ServicesRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Customer Products and ServicesRestProcessor.class.getName());
